...
void World_Gazebo::Load(physics::WorldPtr _world, sdf::ElementPtr _sdf)
{
    ...
    // Publishers
    gazebo2world_pub_ = rosnode_->advertise<allocation_common::gazebo2world_info>("allocation_gazebo/gazebo2world_info",MAXNUM_AGENT);
    debug_pub_ = rosnode_->advertise<std_msgs::Float64MultiArray>("debug",MAXNUM_AGENT);
...
void World_Gazebo::terminal_info_CB(const allocation_common::terminal2gazebo_info::ConstPtr &_msg)
{
    ...
    for(int i=0;i<_msg->robot_pos_x.size();i++)
    {
        ...
        msgs::Set(factory_msg.mutable_pose(),
                  ...
                  ignition::math::Vector3d(_msg->robot_pos_x[i],_msg->robot_pos_y[i],0),
...
bool World_Gazebo::update_model_info(void)
{
    ...
    if(ModelStatesCB_flag_)
    {
        for(int i=0; i<model_count_;i++)
        {
            ...
            {
                int task_id= atoi(model_states_.name[i].substr(task_name.size(),3).c_str());
            ...
            else if(model_states_.name[i].compare(0, robot_name.size(), robot_name)==0)
            {
                ...
                int robot_id = atoi(model_states_.name[i].substr(robot_name.size(),2).c_str());
