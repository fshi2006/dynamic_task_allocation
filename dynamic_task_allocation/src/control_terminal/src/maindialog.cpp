    ...
    ui(new Ui::MainDialog)
{
    ...
    start_pub = nh_.advertise<std_msgs::Char>("start_msgs",50);
    ...
    terminal2robots_info_->greedorprobability=0;
...
MainDialog::~MainDialog()
{
    ...
    if(is_robot_exsit_)
    {
        ...
        // pidof 是一个shell命令,显示关键字包含task_allocation_node的pid,返回值为pid的list
        fp=popen("pidof task_allocation_node","r");
        char *tmp=fgets(pid_line,5000,fp); //把这些pid存到pid_line中

        //kill the pids
        char pid_kill[10000];
        strcpy(pid_kill, "kill "); //字符串复制
        strcat(pid_kill, pid_line); // 两个字符串连接起来
        fp=popen(pid_kill,"r");
        pclose(fp);
    }
}
...
bool MainDialog::init_robots()
{
    ...
    //     FILE *fp = NULL;
	// //std::string full_path = ros::package::getPath("dynamic_task_allocation") + "/src/allocation_common/spawn_robots.sh";
    //     //fp = popen(full_path.c_str(),"w");
	// // fp = popen("/home/luyaomin/catkin_planning/1dynamic_task_allocation/src/spawn_robots.sh","w");
    // //fp = popen("/home/luyaomin/catkin_planning/1dynamic_task_allocation/src/spawn_robots_local.sh","w");
    // FILE *fp1 = NULL;
    // // fp1 = popen("/home/luyaomin/catkin_planning/1dynamic_task_allocation/src/allocation_local.sh","w");
    //     if(fp == NULL || fp1 == NULL)
    //         return false;
    //     pclose(fp);
    //     pclose(fp1);
...
bool MainDialog::on_init_map_clicked()
{
    ...
    std::string full_path = ros::package::getPath("control_terminal") + "/src/Nodes.txt";
    ...
    for (int i = 0; i < num_tasks_; i++)
    {
        ...
        terminal2gazebo_info_->task_pos_x.push_back(tempX);
        terminal2gazebo_info_->task_pos_y.push_back(tempY);
    ...
    for(int i=0;i<num_robots_;i++)
    {
        ...
        if(!ui->random_pos->isChecked())
        {
            if(i<10)
            {
                terminal2gazebo_info_->robot_pos_x.push_back(-tmp_map/2-1);
                terminal2gazebo_info_->robot_pos_y.push_back(-tmp_map/2+i);
            }
            else if(i<20)
            {
                terminal2gazebo_info_->robot_pos_x.push_back(-tmp_map/2-2);
                terminal2gazebo_info_->robot_pos_y.push_back(-tmp_map/2+i-10);
            }
            else if(i<30)
            {
                terminal2gazebo_info_->robot_pos_x.push_back(-tmp_map/2-3);
                terminal2gazebo_info_->robot_pos_y.push_back(-tmp_map/2+i-20);
            }
            else if(i<40)
            {
                terminal2gazebo_info_->robot_pos_x.push_back(-tmp_map/2-4);
                terminal2gazebo_info_->robot_pos_y.push_back(-tmp_map/2+i-30);
            }
            else if(i<50)
            {
                terminal2gazebo_info_->robot_pos_x.push_back(-tmp_map/2-5);
                terminal2gazebo_info_->robot_pos_y.push_back(-tmp_map/2+i-40);
            }
        }
        ...
        terminal2robots_info_->all_allocation_robot_info[i].robot_power=10;
    }
...
void MainDialog::on_start_pause_clicked()
{
    if(terminal2robots_info_->allocation_mode==ALLOCATION_STOP)
    {
        ...
        terminal2robots_info_->allocation_mode=ALLOCATION_START;
        std_msgs::Char start_state;
        start_state.data = ALLOCATION_START;
